export function nsopatchParser(code: string) {
    ...
    if (!isValid) throw new Error("[nsopatchParser]: Failed to parse patch, invalid syntax?");
    ...
    for (const line of code.split(";")) {
        ...
        if (location.length !== 8 || !machineCode || !location || (machineCode.length % 2)) throw new Error("[nsopatchParser]: Failed to parse patch, invalid syntax?");
